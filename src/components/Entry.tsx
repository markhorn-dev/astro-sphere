...
type Props = {
  tags: string[]
  data: CollectionEntry<"entries">[]
}

export default function Blog({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [entries, setEntries] = createSignal<CollectionEntry<"entries">[]>([])

  createEffect(() => {
    setEntries(data.filter((entry) => 
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm uppercase mb-2">
            SHOWING {entries().length} OF {data.length} POSTS
          </div>
